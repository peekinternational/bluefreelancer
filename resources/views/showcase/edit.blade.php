...
@section('content')
    ...
    <section class="container pb-5">
        ...
        <form action="{{ route('showcase.update', $showcase->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group image-uploader-wrap mb-4">
                        ...
                        <label class="image-uploader" for="imageUploader">
                            <div class="image-uploader-overlay" id="imageOverlay"
                                ...
                                <img src="{{ $showcase ? url('uploads/showcases/' . $showcase->img) : '#' }}" alt="Img"
                                    id="imgOutput">
                            </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select id="selectShowcase" name="cate" class="custom-select">
                            ...
                            <option value="Logo" {{ $showcase->cate == 'Logo' ? 'selected' : '' }}>Logo</option>
                            <option value="Websites" {{ $showcase->cate == 'Websites' ? 'selected' : '' }}>Websites
                            </option>
                            <option value="Mobile App" {{ $showcase->cate == 'Mobile App' ? 'selected' : '' }}>Mobile App
                            </option>
                            <option value="Graphic Design" {{ $showcase->cate == 'Graphic Design' ? 'selected' : '' }}>
                                Graphic Design</option>
                            <option value="Illustration" {{ $showcase->cate == 'Illustration' ? 'selected' : '' }}>
                                Illustration</option>
                            <option value="3D Model" {{ $showcase->cate == '3D Model' ? 'selected' : '' }}>3D Model
                            </option>
                        </select>
                        @error('cate')
                            <span class="text-danger">{{ $message }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="description" id="detailsShowcase" cols="30"
                            rows="9">{{ $showcase->description }}</textarea>
                        @error('description')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <select id="currencyShowcase" name="currency" class="custom-select">
                                    <option value="USD" {{ $showcase->currency == 'USD' ? 'selected' : '' }}>USD</option>
                                    <option value="KRW" {{ $showcase->currency == 'KRW' ? 'selected' : '' }}>KRW</option>
                                </select>
                                @error('currency')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
            ...
            <div class="row">
                ...
                <div class="col-md-4 text-right">
                    <input type="submit" class="btn btn-secondary font-size-sm" value="{{ __('UpdateShowcase') }}">
                </div>
            </div>
        </form>
    </section>
